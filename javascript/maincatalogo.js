...
let manubrio = new productoDetalles(
  "Manubrio",
  "AC-012",
  8,
  10,
  (src = "../imag/manubrio.jpg")
);
let asiento = new productoDetalles(
  "Asiento",
  "AE-017",
  9,
  5,
  (src = "../imag/asiento.jpg")
);
let caucho = new productoDetalles(
  "Caucho Taco",
  "AA-003",
  7,
  12,
...
let pedal = new productoDetalles(
  "Pedal grande",
  "AA-004",
  4,
  20,
...
for (let producto of productos) {
  ...
  function agregarProducto() {
    ...
    carritoProducto = carrito.filter(
      (compra) => compra.producto == producto.producto
    );
...
for (let producto of productos) {
  let removerProducto = document.getElementById(
    `quitarBtn${producto.producto}`
  );
...
formulario.addEventListener("submit", (e) => {
  ...
  if (
    ...
    // Agregar caucho
    carritoCaucho = carrito.filter(
      (compra) => compra.producto == "Caucho Taco"
    );
    ...
    // Agregar pedal
    carritoPedal = carrito.filter(
      (compra) => compra.producto == "Pedal grande"
    );
    ...
    $(".carritoFinal").append(
      `<div class= "lineaFinal" style="font-weight: bold">TOTAL DE LA COMPRA = ${totalTotal}U$D</div>`
    );
    $(".lineaFinal").css("color", "red").slideUp(2000).slideDown(2000);

    const URL_DOLAR = "https://criptoya.com/api/dolar";
    $(".carritoFinal").append(
      '<button type="button" id= btnDolar class="btn_volver btn btn-warning">Precio en pesos</a> </button>'
    );
    $("#btnDolar").click(() => {
